...
import { OpeningBalance,OpeningBalancePayload } from "../types/OpeningBalance";
...
export const useOpeningBalance = (): any => {
  ...
  const cancelDialog = () => {
    data.formData = {
      items: [
        { funding_source_id: 1999, amount: 0.00 }
      ]};
  ...
  const addItem = () => {
    data.formData.items.push({
      funding_source_id: 99999, amount: 0.00
    })
  }
  ...
  const getData = (params: any) => {
    ...
    data.formData.items = [{funding_source_id: 1 ,
      amount:0.0},{funding_source_id: 2 ,
      amount:0.3}];
  };
