...
export const useInvoice = (): Record<string, unknown> => {
  ...
  const approveInvoiceRevDialog = (reversalId: any) => {
    data.approvemodal = !data.modal;
    data.itemToReverse = reversalId;
  };

  const approveInvoiceReversal = () => {
    console.log("itemToReverse", data.itemToReverse);
  };
