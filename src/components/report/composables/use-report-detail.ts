...
import { findReport, fetchReportParams } from "../services/report.services";
...
export const useReportDetail = () => {
  ...
  const init = () => {
    const location_id = router.currentRoute.params.location_id;
    ...
    if (report_id) {
      ...
      router.push({ path: `/reports/` }); // -> /reports/2
    }
  };
