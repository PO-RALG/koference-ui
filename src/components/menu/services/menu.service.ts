...
const getPermissionsByResource = async (id: number | string, resource: string) => {
  return await axios.get(`${RESOURCES_API}/${id}/get-permissions?resource=${resource}`);
};
...
export { get, find, create, update, deleteEntry, addPermissions, getResourceCategories, getPermissionsByResource };
